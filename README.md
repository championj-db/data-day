...
You'll stand up a small, governed superannuation dataset — three tables, a metric view, an
analytical view, and an AI/BI dashboard — and then use it to build and configure a **Genie space**.
...
| `v_super_illiquidity_peer` | view | Per-fund illiquid % vs peer-segment averages |
...
1. Go to **Genie → New** and scope it to the **`workspace.data_day`** schema
   (add the 3 tables and the two views).
...
- **Metric views** need a recent serverless runtime. If yours is older, the notebook automatically
  falls back to a plain view with the same per-fund calculations; the dashboard works either way.
